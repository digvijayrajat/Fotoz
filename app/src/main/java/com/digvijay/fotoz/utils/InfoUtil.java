...
public class InfoUtil {
    ...
    public static InfoItem retrieveDimensions(Context context, ExifInterface exif) {
    ...
    public static InfoItem retrieveFormattedDate(Context context, ExifInterface exif) {

            String dateString = String.valueOf(ExifUtil.getCastValue(exif, ExifInterface.TAG_DATETIME));
            try {
                Date date = new SimpleDateFormat("yyyy:MM:dd HH:mm:ss", Locale.US).parse(dateString);
                String formattedDate = new SimpleDateFormat("EEE d MMM yyyy HH:mm", Locale.US).format(date);
                return new InfoItem(context.getString(R.string.info_date), formattedDate);
            } catch (ParseException ignored) {

                String formattedDate = new SimpleDateFormat("EEE d MMM yyyy HH:mm", Locale.US).format("000 3 JAN 2019 12:22");
                return new InfoItem(context.getString(R.string.info_date), formattedDate);
    ...
    public static InfoItem retrieveFocalLength(Context context, ExifInterface exif) {
        ...
        if (focalLengthObject != null) {
            ...
            CalculateRational r = CalculateRational.Companion.parseRational(focalLength);
            if (r != null) {
    ...
    private static class Parser {
        ...
        private static String parseFileSize(Context context, long fileBytes) {
            ...
            String size = String.format(Locale.US, "%.3f", bytes);
            return size + unit;
        }
        ...
        private static String parseGPSLongOrLat(String input, boolean positive) {
            ...
            for (int i = 0; i < parts.length; i++) {
                CalculateRational r = CalculateRational.Companion.parseRational(parts[i]);
                int factor = 1;
                for (int k = 0; k < i; k++) {
                    factor *= 60;
                }
